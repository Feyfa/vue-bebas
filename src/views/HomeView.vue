<template>
  <div>
    ...
    <button style="font-size: 3rem; margin-left: 1rem" @click="ubahNull">ubah null</button>
    <button style="font-size: 3rem; margin-left: 1rem" @click="ubahAda">ubah ada</button>
  </div>
</template>
...
export default {
  data() {
    return {
      userData: {
        ...
        locatorurl: 'searchid',
        enhancename: 'Enhance ID Jidan',
        enhanceurl: 'enhanceidjidan'
      },
      sidebarMenuRoot: {
        local: {
          name: 'Site ID',
  ...
  methods: {
    ...
    ubahNull() {
      ...
      data.enhancename = null;
      data.enhanceurl = null;
      localStorage.setItem('userData', JSON.stringify(data));
    },

    ubahAda() {
      let data = JSON.parse(localStorage.getItem('userData'));
      data.enhancename = this.userData.enhancename;
      data.enhanceurl = this.userData.enhanceurl;
      localStorage.setItem('userData', JSON.stringify(data));
    }
